...
struct Args {
    ...
    #[arg(short = 'P', long)]
    disable_run_nu_path_traversal_check: bool,

    #[arg(short = 'S', long)]
    disable_run_nu_system_dir_check: bool,
}
...
fn test_default_args() {
    ...
    assert!(!args.enable_run_nu);
    assert!(!args.disable_run_nu_path_traversal_check);
    assert!(!args.disable_run_nu_system_dir_check);
}
...
#[test]
fn test_enable_run_flag() {
    let args = Args::try_parse_from(["nu-mcp", "--enable-run-nu"])
        .expect("Failed to parse --enable-run-nu flag");
...
fn test_security_filter_long_flags() {
    let args = Args::try_parse_from([
        "nu-mcp",
        "--disable-run-nu-path-traversal-check",
        "--disable-run-nu-system-dir-check",
    ])
...
fn test_extension_mode_typical_usage() {
    ...
    assert!(!args.enable_run_nu); // Default disabled with tools_dir
}
