...
def main [] {
  print "Running tmux tool tests...\n"

  # Verify we're in the project root
  if not ("tools/tmux/mod.nu" | path exists) {
  ...
  let nu_lib_dirs = if "NU_LIB_DIRS" in $env {
    # Convert list to string with proper separator
    $env.NU_LIB_DIRS | str join ":"
  } else {
    null
  }

  # Discover all test files
  let test_files = glob tools/tmux/tests/test_*.nu

  if ($test_files | is-empty) {
    print "No test files found (looking for tools/tmux/tests/test_*.nu)"
    exit 0
  }
