...
export def find [] {
  try {
    let namespaces = (
      kubectl get ns -l app.kubernetes.io/part-of=argocd -o jsonpath='{.items[*].metadata.name}'
      | str trim
    )
...
export def resolve [args: record] {
  ...
  let current_ns = try {
    kubectl config view --minify -o jsonpath='{.contexts[0].context.namespace}' | str trim
  } catch { "" }
